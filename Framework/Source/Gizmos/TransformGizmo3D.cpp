...
void TransformGizmo3D::UpdateTransformGizmo(Scene* pActiveScene)
{
    ...
    if (m_pActiveGameObject && m_pActiveGameObject->GetTransform() && m_pCamera)
    {
...
{
    if( m_pActiveGameObject->GetTransform() == nullptr )
        return;
...
void TransformGizmo3D::SyncPhysicsBodyToTransform()
{
    Component* pComponent = m_pActiveGameObject->GetFirstComponentOfType(CollisionComponent::GetStaticType());
    if (pComponent)
    {
